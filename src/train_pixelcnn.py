...
def runExperiment():
    ...
    if config.PARAM['resume_mode'] == 1:
        ...
        last_epoch = 1
        current_time = datetime.datetime.now().strftime('%b%d_%H-%M-%S')
        logger_path = 'output/runs/train_{}_{}'.format(config.PARAM['prior_tag'], current_time) if config.PARAM[
    ...
    for epoch in range(last_epoch, config.PARAM['num_epochs'] + 1):
        logger.safe(True)
        train(data_loader['train'], model, prior, optimizer, logger, epoch)
        test(data_loader['train'], model, prior, logger, epoch)
        ...
        if config.PARAM['save_mode'] >= 0:
            ...
            if config.PARAM['pivot'] > logger.mean[config.PARAM['pivot_metric']]:
                ...
                shutil.copy('./output/model/{}_checkpoint.pt'.format(config.PARAM['prior_tag']),
                            './output/model/{}_best.pt'.format(config.PARAM['prior_tag']))
        logger.reset()
    logger.safe(False)
    return


def train(data_loader, model, prior, optimizer, logger, epoch):
    metric = Metric()
    model.train(True)
    for i, input in enumerate(data_loader):
        ...
        if i % int((len(data_loader) * config.PARAM['log_interval']) + 1) == 0:
            ...
            info = {'info': ['Model: {}'.format(config.PARAM['prior_tag']),
