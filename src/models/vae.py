...
def loss(input, output):
    recons_loss = F.mse_loss(output['img'], input['img'])
    kld_loss = torch.mean(-0.5 * torch.sum(1 + output['logvar'] - output['mu'] ** 2 - output['logvar'].exp(), dim=1),
                          dim=0)
