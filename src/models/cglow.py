...
class ActNorm(nn.Module):
    def __init__(self, hidden_size, logdet=True):
        super().__init__()
        self.loc = nn.Parameter(torch.zeros(1, hidden_size, 1, 1))
        self.scale = nn.Parameter(torch.ones(1, hidden_size, 1, 1))
...
class InvConv2dLU(nn.Module):
    def __init__(self, hidden_size):
        super().__init__()
        weight = np.random.randn(hidden_size, hidden_size)
        q, _ = la.qr(weight)
...
class AffineCoupling(nn.Module):
    def __init__(self, input_size, hidden_size=512, affine=True):
        super().__init__()
        self.affine = affine
        self.net = nn.Sequential(
            nn.Conv2d(input_size // 2, hidden_size, 3, padding=1),
            ActNorm(hidden_size, logdet=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(hidden_size, hidden_size, 1),
            ActNorm(hidden_size, logdet=False),
            nn.ReLU(inplace=True),
            ZeroConv2d(hidden_size, input_size if self.affine else input_size // 2),
        )
...
class Flow(nn.Module):
    def __init__(self, input_size, hidden_size, affine=True, conv_lu=True):
        super().__init__()
        ...
        self.coupling = AffineCoupling(input_size, hidden_size, affine=affine)
...
class Block(nn.Module):
    def __init__(self, input_size, hidden_size, K, split=True, affine=True, conv_lu=True, num_mode=None):
        ...
        for i in range(K):
            self.flows.append(Flow(squeeze_dim, hidden_size, affine=affine, conv_lu=conv_lu))
        self.split = split
        if split:
            self.prior = ZeroConv2d(input_size * 2, input_size * 4)
        else:
            self.prior = ZeroConv2d(input_size * 4, input_size * 8)
        self.embedding = ZeroConv2d(num_mode, input_size * 8, 1, 1, 0)
    ...
    def forward(self, input, indicator):
        ...
        if self.split:
            ...
            h = self.prior(zero)
            h_indicator = self.embedding(indicator.view([*indicator.size(), 1, 1]))
            h += h_indicator
...
class CGlow(nn.Module):
    def __init__(self, data_shape, hidden_size, K, L, affine=True, conv_lu=True, num_mode=None):
        super().__init__()
        ...
        for i in range(L - 1):
            self.blocks.append(Block(input_size, hidden_size, K, True, affine, conv_lu, num_mode))
            input_size *= 2
        self.blocks.append(Block(input_size, hidden_size, K, False, affine, conv_lu, num_mode))
