...
@Usage:
    # CMD
        ...
        python inception_score_official_tf.py
        /data4/linziheng/datasets/fashionai-attributes-challenge-baseline/fashionAI_attributes_test/test/Images
        /coat_length_labels/
...
def get_inception_score(images, splits=10, bs=256):
    ...
    with tf.Session() as sess:
        ...
        for i in range(n_batches):
            ...
            pred = sess.run(softmax, {'InputTensor:0': img})
            preds.append(pred)
...
def _init_inception():
    ...
    with tf.gfile.GFile(os.path.join(MODEL_DIR, 'classify_image_graph_def.pb'), 'rb') as f:
        graph_def = tf.GraphDef()
        graph_def.ParseFromString(f.read())
...
if __name__ == '__main__':
    ...
    def cal_on_cifar10_train():
        ...
        def unpickle(file):
            ...
            return {'x': d['data'].reshape((len(d['data']), 3, 32, 32)),
                    'y': np.array(d['labels']).astype(np.uint8)}

            # normalized to -1 ~ +1
            # return {'x': np.cast[np.float32]((-127.5 + d['data'].reshape((10000, 3, 32, 32))) / 128.).transpose(
            # 0, 2, 3, 1),
            #         'y': np.array(d['labels']).astype(np.uint8)}
        ...
        train_x, train_y = load('./data/cifar10', subset='train')
